...
reload_nginx() {
    ...
    docker compose exec -T reverseproxy nginx -t -c /etc/nginx/nginx.conf
    if [ $? -ne 0 ]; then
    ...
    docker compose exec -T reverseproxy nginx -s reload || echo "Failed to reload Nginx, please check the container status and logs."
}
